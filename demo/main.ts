...
bind('s-darkspring', 'v-darkspring', (v) => engine.updateSettings({ darkSpringBoost: v }));
bind('s-damping', 'v-damping', (v) => engine.updateSettings({ damping: v }));
bind('s-maxdisp', 'v-maxdisp', (v) => engine.updateSettings({ maxDisplacement: v }));

// Water field
bind('s-nscale', 'v-nscale', (v) => { field.settings.noiseScale = v; });
bind('s-tspeed', 'v-tspeed', (v) => { field.settings.timeSpeed = v; });
bind('s-sharp', 'v-sharp', (v) => { field.settings.sharpness = v; });

// Ripples
bind('s-ramp', 'v-ramp', (v) => { ripple.settings.amplitude = v; });
bind('s-rwlen', 'v-rwlen', (v) => { ripple.settings.wavelength = v; });
bind('s-rdecay', 'v-rdecay', (v) => { ripple.settings.decay = v; });
bind('s-rforce', 'v-rforce', (v) => { ripple.settings.force = v; });
